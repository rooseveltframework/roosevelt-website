// this file builds the roosevelt website, serves it, and rebuilds it as you edit
//
// run it to do that:
//   node test-server.js                 build the site, serve it, and rebuild the pages you edit as you edit them
//   node test-server.js --production-mode   build and serve without watching
//   node test-server.js --build         write the site out and stop there, which is what `npm run build` does
//   node test-server.js --fast-mode     build only the current version of each module's docs, which is most of the build
//
// require it to get at the pieces the test suite checks; requiring it builds nothing, since the build only runs when this file is the one node was pointed at
      ...
      for (const version of thisReposVersions) {
        ...
        if (version !== 'Older...') {
          ...
          if (!fs.existsSync(`${source}.html`) && !fs.existsSync(path.join(source, 'index.html'))) {
            newUrl = ('/docs/' + fileParts.join('/') + '/' + (isLatest ? 'latest' : versionWithoutLatest)).replace('//', '/')
          }
        }
...
async function build () {
  await require('roosevelt')(rooseveltParams()).init()
  finishBuild()
}
...
// the part of a build that is not roosevelt's to do
...
if (require.main === module) {
  ...
  ;(async () => {
    ...
    // roosevelt builds the site, and in development mode it also watches the files it was built from and renders the pages an edit affects
